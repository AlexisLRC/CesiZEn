                ...
                <form action="{{ route('personal.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-3 gap-6 mb-8">
                        <div>
                            ...
                            <input type="number" name="duration_inhale" value="{{ $exercise->duration_inhale ?? 5 }}" class="w-full border-gray-300 rounded-lg text-center" required>
                        </div>
                        <div>
                            <label class="block text-sm text-center text-gray-600 font-bold mb-2">Apnée</label>
                            <input type="number" name="duration_hold" value="{{ $exercise->duration_hold ?? 0 }}" class="w-full border-gray-300 rounded-lg text-center" required>
                        </div>
                        <div>
                            <label class="block text-sm text-center text-green-600 font-bold mb-2">Expiration</label>
                            <input type="number" name="duration_exhale" value="{{ $exercise->duration_exhale ?? 5 }}" class="w-full border-gray-300 rounded-lg text-center" required>
                        </div>
                    </div>
